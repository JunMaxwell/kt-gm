...
/** Like `onInt`, but the inspector works in half-inches. */
const onNum = (fn: (n: number) => void) => (raw: string) => {
  const v = parseFloat(raw)
  if (Number.isFinite(v)) fn(v)
}

type Sel = { kind: 'piece' | 'op'; id: string } | null
type Drag = { kind: 'piece' | 'op' | 'marker'; id: string; x: number; y: number; ox: number; oy: number }
 ...
 */
function MapBuilder({ game, dispatch }: { game: Game; dispatch: Dispatch }) {
  ...
  return (
    <details open className="mx-4 mb-4 overflow-hidden rounded-xl border border-rule bg-paper shadow-sm">
      <summary className="display cursor-pointer bg-ink px-3 py-2 text-xl text-white">Board</summary>
      ...
      <div className="grid gap-4 p-3 xl:grid-cols-[minmax(0,1fr)_17rem]">
  ...
  if (net.viewer)
    return (
      <div className="min-h-screen">
        ...
        <div inert>
          <Console game={game} dispatch={dispatch} net={net} editing={false} setEditing={() => {}} canUndo={false} />
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen">
      ...
      <footer className="mx-4 mb-8 text-center text-xs leading-relaxed text-ink/45">
        Card layout and colour palette after{' '}
